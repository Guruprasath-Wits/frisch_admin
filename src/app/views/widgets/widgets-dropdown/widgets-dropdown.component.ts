...
export class WidgetsDropdownComponent implements OnInit, AfterContentInit {
  ...
  getUsersCount(): void {
    this.adminservice.loadUsers().subscribe(
      (response: any) => {
        ...
        this.usersCount = response.user.length; // Get the total count of users
      },
      error => {
  ...
  getCategoriesCount(): void {
    this.adminservice.getCategory({}).subscribe(
      (response: any) => {
        if (response.status) {
          this.categoriesCount = response.category.length; // Adjust based on API response
        } else {
  ...
  getProductsCount(): void {
    this.adminservice.fetchProducts().subscribe(
      (response) => {
        if (response.status) {
          this.productsCount = response.product.length; // Get the total count of products
        }
      },
      (error) => {
  ...
  getPendingOrdersCount(): void {
    this.adminservice.loadOrders().subscribe(
      (response: any) => {
        this.pendingOrdersCount = response.orders.filter((order: any) => order.status === 'pending').length;
      },
      (error) => {
